...
from experimental_analysis.viz_params import OneLTwoDVizParams
from experimental_analysis.viz_organisms import (
...
class VisualizationScene(ThreeDScene):
    ...
    def construct(self):
        ...
        # Replication: experiment settings for reproducibility
        repl_lines = getattr(type(self), "panel_replication_lines", None)
        if repl_lines:
            repl_header = Text("Replication", font=panel_font, color=BLACK, font_size=font_size_section)
        ...
        if levers_data and levers_colors:
            ...
            dar_keys = ["elimination_rate", "selection_pressure", "mutation_std", "point2_x", "point2_y", "point4_x", "point4_y"]
            short_lam = ["bw", "tgs", "msf", "mw", "ds", "minm", "maxm", "no", "fgrb"]
            short_dar = ["er", "sp", "mut", "p2x", "p2y", "p4x", "p4y"]
            ...
            lam_table_header = Text(
                "seed | " + " | ".join(short_lam),
                font=panel_font, color=BLACK, font_size=font_size_table,
            )
            lam_table_header.next_to(panel[-1], DOWN, buff=0.04).align_to(panel_title, LEFT)
            panel.add(lam_table_header)
            for idx, row in enumerate(levers_data):
            ...
            dar_table_header.next_to(panel[-1], DOWN, buff=0.04).align_to(panel_title, LEFT)
        ...
        panel.to_edge(RIGHT).shift(UP * 0.8)
