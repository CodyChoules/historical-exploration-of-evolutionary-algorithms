...
def generate_organism_vectors(parent1_start, parent1_end, parent2_start, parent2_end, 
                               use_explicit_sum_method=True, besoin_weight=1.0, 
                               topology_function=None, topology_gradient_scale=0.1):
    """
...
def pure_lamarckian_function(besoin_topology_function, parent1_start, parent1_end, parent2_start, parent2_end, 
                              num_offspring=2, num_generations=10, besoin_weight=1.0, topology_gradient_scale=0.1,
                              magnitude_std_fraction=0.1, direction_std=0.1, min_magnitude=0.01):
    """
    ...
    Args:
        ...
        parent1_start: numpy array, start point of first parent vector (initial generation)
        parent1_end: numpy array, end point of first parent vector (initial generation)
        parent2_start: numpy array, start point of second parent vector (initial generation)
        parent2_end: numpy array, end point of second parent vector (initial generation)
        ...
        magnitude_std_fraction: float, standard deviation as fraction of mean magnitude for variation (default: 0.0 = deterministic)
    ...
    for generation in range(num_generations):
        ...
        for i in range(num_offspring):
            ...
            # Calculate besoin vector for THIS child organism from topology gradient at its origin
            gradient = calculate_gradient(besoin_topology_function, child_origin[:2])
            # Negative gradient points toward minimum (steepest descent)
            besoin_vector_2d = -gradient * topology_gradient_scale
            besoin_vector = np.array([besoin_vector_2d[0], besoin_vector_2d[1], 0])
            ...
            if magnitude_std_fraction > 0:
                magnitude_std = mean_magnitude * magnitude_std_fraction
                child_magnitude = np.random.normal(mean_magnitude, magnitude_std)
                child_magnitude = max(min_magnitude, child_magnitude)
            else:
                child_magnitude = mean_magnitude
        ...
        if generation < num_generations - 1 and len(organisms) >= 2:
...
# Topology functions for testing (can be imported from elsewhere if needed)
