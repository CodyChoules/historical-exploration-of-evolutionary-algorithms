...
from optimizationfunctions.evolutionalgorithms.darwinianfunctions import (
...
def summarize_final_state(results: Dict[str, Any]) -> Dict[str, float]:
    ...
    if darwinian:
        ...
        survivors = np.array(last_generation.get("survivors", []), dtype=float)
        if len(survivors) > 0:
            survivor_mean = np.mean(survivors[:, :2], axis=0)
            summary["darwinian_mean_x"] = float(survivor_mean[0])
            summary["darwinian_mean_y"] = float(survivor_mean[1])
            summary["darwinian_survivor_count"] = float(len(survivors))
