"""
Test MD2: Meta-optimized (via Darwinian-tuned) Lamarckian v Darwinian.
...
MD2_META_ELITE = 10
# Align meta-evaluation seeds with final-run seeds so selected levers are scored
# under the same stochastic context used in the final replay.
MD2_LAM_SEED_OFFSET = 50
MD2_DAR_SEED_OFFSET = 100
# Meta objective now aligns with reported evaluation metric.
MD2_META_OBJECTIVE_METRIC = "best_fitness"
# Force MD2 Darwinian initial organisms to start at the lower-left bound.
MD2_DAR_START_XY = (-12.0, -12.0)
...
def build_initial_population_from_levers(levers: dict, base_points: np.ndarray) -> np.ndarray:
    """Build MD2 Darwinian initial population, anchored at (-12, -12) for all start points."""
    pop = np.asarray(base_points, dtype=float).copy()
    if pop.size == 0:
        return pop
    pop[:, 0] = MD2_DAR_START_XY[0]
    pop[:, 1] = MD2_DAR_START_XY[1]
    if pop.shape[1] >= 3:
        pop[:, 2] = 0.0
    return pop
...
def evaluate_lamarckian_candidate(
    ...
    seed: int,
    call_budget: int,
    objective_metric: str = MD2_META_OBJECTIVE_METRIC,
) -> float:
    """Run Lamarckian with levers; return selected objective score (lower is better)."""
    ...
    if summary is None:
        return float("inf")
    score = summary.get(objective_metric)
    if score is None:
        return float("inf")
    return float(score)
...
def evaluate_darwinian_candidate(
    ...
    """Run Darwinian with levers; return selected objective score (lower is better)."""
...
def meta_optimize_lamarckian(
    ...
    eval_seed_offset: int = MD2_LAM_SEED_OFFSET,
    objective_metric: str = MD2_META_OBJECTIVE_METRIC,
    verbose: bool = False,
) -> tuple[dict, float, list[dict]]:
    """Return (best_lamarckian_levers, best_score, history) for objective_metric."""
    ...
    for g in range(num_gens):
        ...
        gen_best = min(x[0] for x in scores) if scores else float("inf")
        gen_avg = (sum(x[0] for x in scores) / len(scores)) if scores else float("inf")
        history.append(
            {
                "generation": int(g),
                "generation_best_score": float(gen_best),
                "generation_avg_score": float(gen_avg),
                "best_score_so_far": float(best_score),
            }
        )

        if verbose:
            print(f"  meta_lam gen={g} best={best_score:.4f} avg={gen_avg:.4f} metric={objective_metric}")
    ...
    return best_levers or sample_lamarckian_levers(rng), best_score, history
...
def meta_optimize_darwinian(
    ...
    num_gens: int = MD2_META_GENERATIONS,
    elite: int = MD2_META_ELITE,
    eval_seed_offset: int = MD2_DAR_SEED_OFFSET,
    objective_metric: str = MD2_META_OBJECTIVE_METRIC,
    verbose: bool = False,
) -> tuple[dict, float, list[dict]]:
    """Return (best_darwinian_levers, best_score, history) for objective_metric."""
    ...
    for g in range(num_gens):
        ...
        if verbose:
            print(f"  meta_dar gen={g} best={best_score:.4f} avg={gen_avg:.4f} metric={objective_metric}")
    ...
    return best_levers or sample_darwinian_levers(rng, n), best_score, history


def run_md2(
    seed: int = 42,
    call_budget: int = MD2_CALL_BUDGET,
    meta_call_budget: int = MD2_META_CALL_BUDGET,
    verbose: bool = True,
    lamarckian_vectors: int = 2,
    darwinian_population: int = 4,
    meta_verbose: bool = False,
    meta_objective_metric: str = MD2_META_OBJECTIVE_METRIC,
) -> dict:
    """
    Run MD2 for one seed: meta-optimize both algorithms, then main run with best levers.
    Returns dict compatible with run_up1 (plus meta outputs and post-meta verification).
    """
    ...
    if lamarckian_vectors == 1 and darwinian_population == 2:
        darwinian_population = 2
    ...
    lam_levers, meta_lam_score, meta_lam_history = meta_optimize_lamarckian(
        parent1_start, parent1_end, parent2_start, parent2_end,
        seed=seed,
        rng=rng,
        call_budget=meta_call_budget,
        eval_seed_offset=MD2_LAM_SEED_OFFSET,
        objective_metric=meta_objective_metric,
    ...
    dar_levers, meta_dar_score, meta_dar_history = meta_optimize_darwinian(
        initial_pop_meta,
        seed=seed,
        rng=rng,
        call_budget=meta_call_budget,
        eval_seed_offset=MD2_DAR_SEED_OFFSET,
        objective_metric=meta_objective_metric,
        verbose=meta_verbose,
    )
    ...
    lam_verify_score = float(lam_sum.get(meta_objective_metric)) if lam_sum else float("inf")
    dar_verify_score = float(dar_sum.get(meta_objective_metric)) if dar_sum else float("inf")

    n_init = initial_points.shape[0]
    out = {
        "seed": seed,
        "lamarckian_vectors": lamarckian_vectors,
        "darwinian_pop": darwinian_population,
        ...
        "meta_objective_metric": meta_objective_metric,
        "meta_lam_history": meta_lam_history,
        "meta_dar_history": meta_dar_history,
        "post_meta_lam_verify_score": lam_verify_score,
        "post_meta_dar_verify_score": dar_verify_score,
    ...
    if verbose:
        print(
            ...
            f"(meta_score={meta_lam_score:.4f}, verify_{meta_objective_metric}={lam_verify_score:.4f}) | "
            f"Dar: best_f={dar_sum['best_fitness']:.4f} dist={dar_sum['distance_to_optimum']:.3f} calls={dar_sum['function_calls']} "
            f"(meta_score={meta_dar_score:.4f}, verify_{meta_objective_metric}={dar_verify_score:.4f})"
        )
    return out
